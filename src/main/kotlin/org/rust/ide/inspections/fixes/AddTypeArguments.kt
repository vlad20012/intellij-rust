...
class AddTypeArguments(element: RsElement) : LocalQuickFixAndIntentionActionOnPsiElement(element) {
    ...
    override fun invoke(
        ...
        addTypeArguments(element, editor)
    }

    companion object {
        fun addTypeArguments(element: RsElement, editor: Editor?, onFinish: (() -> Unit)? = null) {
            val inserted = insertTypeArgumentsIfNeeded(element)
            if (inserted == null) {
                onFinish?.invoke()
            } else {
                // TODO: replace the generated placeholders with actual types in tests?
                if (isUnitTestMode) {
                    onFinish?.invoke()
                } else {
                    editor?.buildAndRunTemplate(element, inserted.map { it.createSmartPointer() }, TemplateResultListener {

...
class MacroExpander(val project: Project) {
    fun expandMacroAsText(def: RsMacro, call: RsMacroCall): Pair<CharSequence, CompactRangeMap>? {
    ...
    private fun substituteMacro(root: PsiElement, subst: WithParent): Pair<CharSequence, CompactRangeMap>? {
        val sb = StringBuilder()
        val ranges = SmartList<MappedTextRange>()
        if (!substituteMacro(sb, ranges, root, subst)) return null
        return sb to CompactRangeMap.from(ranges)
    }
